<template>
  <div class="elegant-container">
    <!-- 页面标题 -->
    <div class="elegant-section database-header">
      <div class="database-title-section">
        <h1 class="elegant-title">🗄️ 数据库技术</h1>
        <p class="elegant-text-light">系统学习数据库原理、SQL优化、事务处理与分布式数据库架构</p>
      </div>
    ...
    <div class="elegant-section">
      ...
      <div class="database-topics-grid">
        <div 
          v-for="topic in filteredTopics" 
          :key="topic.id" 
          class="database-topic-card"
...
<script>
import { databaseTopics, studyPaths, recentStudies } from '../../data/databaseTopics.js'
  ...
  computed: {
    overallStats() {
      ...
      return {
        ...
        overallProgress: 15, // 静态进度
        completedTopics: 1, // 已完成模块数
        studyHours: 8, // 静态数据
        streakDays: 2,  // 静态数据
        weeklyProgress: 3 // 静态数据
      }
    },
    filteredTopics() {
      // 由于删除了进度信息，所有筛选都返回全部主题
      return this.databaseTopics
    }
  },
  methods: {
    handleTopicClick(topic) {
      ...
      // 跳转到知识点详情页
      this.$router.push(`/vivo50/database/topic/${topic.id}`)
    },
    ...
    getTopicName(topicId) {
      const topic = this.databaseTopics.find(t => t.id === topicId)
...
.database-topic-card:hover .action-arrow {
